...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    // Verify recipient is a freelancer (not another client)
    $recipientStmt = $pdo->prepare('SELECT role FROM users WHERE id = ?');
    $recipientStmt->execute([$recipientId]);
    $recipient = $recipientStmt->fetch();

    if (!$recipient || $recipient['role'] !== 'freelancer') {
        echo json_encode(['success' => false, 'message' => 'Invalid recipient.']);
        exit;
    }

    // Verify they have an accepted proposal or existing conversation with this freelancer
    $hasConnectionStmt = $pdo->prepare('
        SELECT 1 FROM proposals p
        JOIN jobs j ON j.id = p.job_id
        WHERE j.client_id = ? AND p.freelancer_id = ? AND p.status = "accepted"
    ...
    if (!$hasConnectionStmt->fetch()) {
        echo json_encode(['success' => false, 'message' => 'You must have an accepted proposal with this freelancer to message them.']);
        exit;
    }
...
$proposalsStmt = $pdo->prepare('
    SELECT DISTINCT p.freelancer_id AS other_user_id, u.full_name, u.avatar, NULL AS last_message_time
    FROM proposals p
    JOIN jobs j ON j.id = p.job_id
